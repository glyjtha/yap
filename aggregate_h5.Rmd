...
# 4) Construct the SeuratIndex
metadata <- metadata %>%
  mutate(SeuratIndex = glue("{plate}-{MultiplexGroup}-A1-{RandomIndex}"))

# 5) Set rownames and inspect assign_ident
rownames(metadata) <- metadata$SeuratIndex

# Now you can look at the assign_ident column:
metadata[ , "assign_ident", drop = FALSE]
```
```{r}
seurat_obj
seurat_obj[["RNA"]]
#seurat_obj <- AddMetaData(seurat_obj, metadata=metadata) //missing metadata data for split_r
